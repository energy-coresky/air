...
class Gate
{
    ...
    function __construct() {
        $this->flags = [
            self::HAS_T3 => 'Address has semantic part',
            self::RAW_INPUT => 'Use raw body input',
            self::AUTHORIZED => 'User must be authorized',
            self::OBJ_ADDR => 'Return address as object',
            self::OBJ_PFS => 'Return postfields as object',
        ];
    }
    
    static function post_data($me = false) {
        global $user;
        if ($me)
            $me->argc($_POST['args']);
        $user->v_sg_prod = isset($_POST['production']);
        $addr = $pfs = [];
        $to =& $addr;
        if (isset($_POST['key'])) {
    ...
    static function load($class = false) {
        $sky_gate = [];
        if (is_file($fn = self::GATE))
            require $fn;
        if (!$class)
            return $sky_gate;
    ...
    static function save($class, $func = false, $ary = false) {
        ...
        if (is_array($func)) { # save virtuals
            ...
            $sky_gate[$class][$func] = true === $ary ? self::post_data() : $ary;
        }
        ...
        file_put_contents(self::GATE, $file);
    }
    ...
    static function real_src(&$class, &$fn_src, $test = true) {
        ...
        if (is_string(self::$contr_ary = $sky_gate[$class])) {
            self::$contr_ary = $sky_gate[$class = self::$contr_ary];
        } elseif ($test) {
            return false;
        }
    ...
    static function compile($class, $func) {
        $me = new Gate;
        json([
            'code' => $me->view_code(self::post_data($me), $class, $func),
            'url'  => PROTO . '://' . $me->url,
        ]);
    }
    ...
    static function cshow() {
        global $user;
        Gate::$cshow = $user->v_sg_cshow;
        if (isset($_POST['s']))
            Gate::$cshow = $user->v_sg_cshow = (int)('true' == $_POST['s']);
        return Gate::$cshow;
    }

    static function view($class, $func = null, $is_edit = true) {
        list($real, $ary) = Gate::load($class); # virt class return real
        if ($real != $class)
    ...
    function code($ary, $cmode, $is_view = true) {
        ...
        if (!$meth && 'main' == $cmode[1] && '' === $cmode[2]) # for main page
            $meth = [1];
        $this->_e = !DEV || $is_view && $user->v_sg_prod ? 'die;' : 'e();';
        $errors = $s0 = '';
    ...
    function process_addr($addr, $flag, $cmode, &$eq0) {
        ...
        $this->url = $user->v_sg_prod ? _PUBLIC . '/' : SNAME . PATH;
