...
/* 2do revision
    ...
    static function start() {
        ...
        if ($sky->s_init_needed) {
            $js = WWW . 'm/' . basename(Install::js);
            file_exists($js) or file_put_contents($js, file_get_contents(Install::js)) or exit("Cannot save `$js`");
            Install::init_reset(sql('+select tmemo from memory where id=5'));
    ...
    static function dummy_gen($table, $size = null) {
        ...
        foreach (sql('@explain $_' . ($table = preg_replace("/`/u", '', $table))) as $c => $r) {
            ...
            elseif (preg_match("/^[a-z]*text$/i", $r[0])) $ary[$c] .= 'Install::dummy_txt();' and $txt = 1;
            elseif ('datetime' == $r[0]) $ary[$c] .= 'date(DATE_DT, time() - rand(0, 3600 * 24 * 7));';
            elseif (preg_match("/varchar\((\d+)\)/", $r[0], $m)) $ary[$c] .= "Install::dummy_txt($m[1] > 15 ? rand(9, $m[1] - 5) : $m[1]);";
